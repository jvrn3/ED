...
main(int argc, char *argv[]){
	...
	char *leitura, *path, *dir, *qry, *ec, *pm, *sistema_viario, *gui;
	
	leitura = path = dir = qry = ec = pm = sistema_viario = gui =NULL;
	...
	while(i < argc){
		...
		if(strcmp("-gui", argv[i]) == 0){
			gui = aloca_tamanho(strlen(argv[i]));
			i++;
	...
	if(gui != NULL){
		...
		destroy(listC);
		destroy(listR);
		printf("\nPrograma executado\nAutores: Joao Vitor R Neto - Matricula: 201600560171\n");
		printf("\t Rafael Yukio Umemoto - Matricula: 201600560364");
